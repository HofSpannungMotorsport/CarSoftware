...
bool HardwareSDCardStressTest() {
    ...
    if (!_added) {
        printf("Adding alive as LoggableValue with %i refreshrate to logger and begin: ", TEST_LOGGER_REFRESH_RATE);
    ...
    printf("Now let the logger run for %i seconds\n", TEST_LOGGER_TIME);
    Timer timer;
    timer.reset();
    timer.start();
