...
class HardwareDisplay : public IDisplay {
    public:
        ...
        virtual void setStatus(display_status_t status) {
            Serial.print("[HardwareDisplay]@parseMessage: status: 0x%");
            Serial.println(status);
        }

        virtual void setSpeed(float speed) {
            Serial.print("[HardwareDisplay]@parseMessage: speed: 0x%");
            Serial.println(speed);
        }

        virtual void setBatteryVoltage(float batteryVoltage) {
            Serial.print("[HardwareDisplay]@parseMessage: batteryVoltage: 0x%");
            Serial.println(batteryVoltage);
        }

        virtual void setCellVoltage(float cellVoltage) {
            Serial.print("[HardwareDisplay]@parseMessage: cellVoltage: 0x");
