...
class CarMessage {
   public:
    ...
    uint8_t get(uint8_t index) const {
        if (index >= STD_CARMESSAGE_DATA_SIZE) return 0;
    ...
    void set(uint8_t value, uint8_t index) {
        if (index >= STD_CARMESSAGE_DATA_SIZE) return;
    ...
    void setLength(uint8_t length) {
        if (length > STD_CARMESSAGE_DATA_SIZE) return;
     ...
     */
    inline void setSenderId(id_device_t senderId) { _senderId = senderId; }
     ...
     */
    inline id_device_t getSenderId() const { return _senderId; }

    /**
     * @brief Set the receiverId for the device which should receive the message. Normally set by
     * the Syncer
     *
     * @param receiverId Set the receiverId for this message
     */
    inline void setReceiverId(id_device_t receiverId) { _receiverId = receiverId; }
     ...
     */
    inline id_device_t getReceiverId() const { return _receiverId; }
     ...
     */
    inline void setMessageId(uint8_t messageId) { _messageId = messageId; }
     ...
     */
    inline uint8_t getMessageId() const { return _messageId; }

    /**
     * @brief Set the Component ID the message is dedicated to. Must be set by the component
     * (SelfSyncable does this automatically).
     *
     * @param componentId The Component ID this message is dedicated to
     */
    inline void setComponentId(id_component_t componentId) { _componentId = componentId; }
     ...
     */
    inline id_component_t getComponentId() const { return _componentId; }

    /**
     * @brief Set if this Message should be prioritized before others on a given Channel
     ...
     */
    inline void setPrioritise(bool prioritise) { _prioritise = prioritise; }
     ...
     */
    inline bool getPrioritise() const { return _prioritise; }

    /**
     * @brief Set if the Message is tracked (send over sTCP)
     *
     * @param isTracked
     */
    inline void setIsTracked(bool isTracked) { _isTracked = isTracked; }

    /**
     * @brief Get if the Message is / should be send over sTCP
     *
     * @param isTracked
     * @return true
     * @return false
     */
    inline bool getIsTracked(bool isTracked) { return _isTracked; }
    ...
    int compareTo(CarMessage &carMessage) {
        ...
        if (_senderId != carMessage.getSenderId()) return 1;
        if (_receiverId != carMessage.getReceiverId()) return 2;
        if (_componentId != carMessage.getComponentId()) return 3;

        // Compare CarMessage Data
        if (_length != carMessage.getLength()) return -1;

        for (uint16_t i = 0; i < _length; i++) {
            if (_data[i] != carMessage.get(i)) return -2 - i;
        }
   ...
   protected:
    ...
    // For sTCP
    uint8_t _messageId = 0;  // ID 0 == untracked

    // For MessageRouter
    id_component_t _componentId = 0;

    // For Flow Control + Flow Control Info
    bool _prioritise = false;  // -> OK Answer has higher prio
    bool _isTracked = false;   // -> Was message send over sTCP Stack?
